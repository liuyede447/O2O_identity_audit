...
def validate_raw_membership(frame: pd.DataFrame, selection_roster: pd.DataFrame, label: str) -> None:
    """Allow selected zero-row images while rejecting unknown images or stratum drift.

    This is a post-access repair for future analyses. It does not alter the
    immutable failed lockbox run or restore confirmatory status.
    """
    required = {"stratum", "image_id"}
    if not required.issubset(frame.columns):
        raise RuntimeError(f"{label} raw table must contain image_id and stratum")
    raw_roster = frame[["stratum", "image_id"]].drop_duplicates()
    if raw_roster["image_id"].duplicated().any():
        raise RuntimeError(f"{label} raw table assigns one image_id to multiple strata")
    expected = selection_roster.set_index("image_id")["stratum"]
    unknown = sorted(set(raw_roster["image_id"]) - set(expected.index))
    if unknown:
        raise RuntimeError(f"{label} raw table contains unselected images: {unknown[:5]}")
    observed = raw_roster.set_index("image_id")["stratum"]
    mismatched = [image_id for image_id, stratum in observed.items() if expected.loc[image_id] != stratum]
    if mismatched:
        raise RuntimeError(f"{label} raw table has selection-stratum mismatches: {mismatched[:5]}")
...
def branch_image_aggregate(
    ...
    validate_raw_membership(fixed, selection_roster, "fixed branch")
    validate_raw_membership(normalized, selection_roster, "normalized branch")
    if fixed.duplicated(keys).any() or normalized.duplicated(keys).any():
        raise RuntimeError("fixed/normalized branch raw table contains duplicate image_id/gt_id keys")
    ...
    common_keys = fixed_keys & normalized_keys
    if not common_keys:
        raise RuntimeError("fixed/normalized common-valid GT intersection is empty")
    fixed_only = fixed_keys - normalized_keys
    normalized_only = normalized_keys - fixed_keys
    fixed = fixed[fixed[keys].apply(tuple, axis=1).isin(common_keys)].copy()
    normalized = normalized[normalized[keys].apply(tuple, axis=1).isin(common_keys)].copy()
    ...
    result.attrs["support_audit"] = {
        "fixed_common_valid": len(fixed_keys),
        "normalized_common_valid": len(normalized_keys),
        "intersection": len(common_keys),
        "fixed_only": len(fixed_only),
...
def pathway_aggregate(
    ...
    for label, frame in (("fixed", fixed), ("normalized", normalized)):
        validate_raw_membership(frame, selection_roster, f"{label} anatomy")
...
def run_self_test() -> dict[str, object]:
    ...
    synthetic_selection = pd.DataFrame(
        [
            ...
            {"stratum": "s_only", "image_id": "no_anatomy_rows"},
        ]
    )
    ...
    fixed_branch_rows = pd.DataFrame(
        [
            ...
            {"stratum": "t_only", "image_id": "valid_image", "gt_id": 1, "common_valid_margin": 1, "size_bin": "s_16_32", "sampling_weight": 2.0, "o2o_fragile": 0, "o2m_fragile": 0},
    ...
    branch_selection = pd.DataFrame(
        [
            ...
            {"stratum": "s_only", "image_id": "no_branch_rows"},
        ]
    )
    ...
    mismatched_support.loc[mismatched_support["gt_id"].eq(1), "common_valid_margin"] = 1
    repaired = branch_image_aggregate(fixed_branch_rows, mismatched_support, branch_selection)
    assert repaired.attrs["support_audit"]["fixed_only"] == 1
    assert repaired.attrs["support_audit"]["intersection"] == 1
    reordered = branch_image_aggregate(
        fixed_branch_rows.iloc[::-1].reset_index(drop=True),
        normalized_branch_rows.iloc[::-1].reset_index(drop=True),
        branch_selection.iloc[::-1].reset_index(drop=True),
    )
    pd.testing.assert_frame_equal(
        branch_clusters.sort_values(["stratum", "image_id"]).reset_index(drop=True),
        reordered.sort_values(["stratum", "image_id"]).reset_index(drop=True),
    )
    duplicate_rows = pd.concat([fixed_branch_rows, fixed_branch_rows.iloc[[0]]], ignore_index=True)
    try:
        branch_image_aggregate(duplicate_rows, normalized_branch_rows, branch_selection)
    except RuntimeError as exc:
        assert "duplicate" in str(exc)
    else:
        raise AssertionError("duplicate branch key was not rejected")
    all_zero = fixed_branch_rows.copy()
    all_zero[["o2o_fragile", "o2m_fragile"]] = 0
    all_zero_clusters = branch_image_aggregate(all_zero, all_zero.copy(), branch_selection)
    assert branch_from_aggregate(all_zero_clusters, "fixed") == {
        "o2o_gap": 0.0,
        "o2m_gap": 0.0,
        "paired_gap": 0.0,
    }
...
def main() -> None:
    ...
    payload = {
        ...
        "branch_support_audit": branch_images.attrs["support_audit"],
