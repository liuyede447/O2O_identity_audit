...
def trace_o2o(criterion, raw: dict[str, torch.Tensor], labels: torch.Tensor, boxes: torch.Tensor, mask: torch.Tensor) -> O2OState:
    ...
    target_gt_idx, fg_mask, post = assigner.select_highest_overlaps(pre, overlaps, assigner.n_max_boxes, align)
    return O2OState(pre.bool(), post.bool(), align, overlaps, target_gt_idx, fg_mask.bool(), anchors, stride, decoded)
