...
def compute_swd(
    ...
    """Compute SWD v3.0 on xyxy boxes (各向异性高斯 2-Wasserstein，无近似，gamma=0 严格退化为NWD)."""
    pred_xywh = xyxy2xywh(pred_bboxes)
    target_xywh = xyxy2xywh(target_bboxes)

    cx_p, cy_p, w_p, h_p = pred_xywh.unbind(-1)
    cx_t, cy_t, w_t, h_t = target_xywh.unbind(-1)

    # sqrt(s) = (w/h)^(gamma/2)，pred 和 gt 用相同映射（对称）
    sq_sp = (w_p / (h_p + eps)).clamp(min=eps) ** (gamma / 2)
    sq_st = (w_t / (h_t + eps)).clamp(min=eps) ** (gamma / 2)

    # 各向异性标准差（长度量纲）
    sw_p = sq_sp * w_p / 2
    sh_p = (h_p / 2) / (sq_sp + eps)
    sw_t = sq_st * w_t / 2
    sh_t = (h_t / 2) / (sq_st + eps)

    center_dist = (cx_p - cx_t).pow(2) + (cy_p - cy_t).pow(2)
    w_dist = (sw_p - sw_t).pow(2)
    h_dist = (sh_p - sh_t).pow(2)
    return torch.sqrt(center_dist + w_dist + h_dist + eps)


def compute_gcd_loss(
    pred_bboxes: torch.Tensor,
    target_bboxes: torch.Tensor,
    eps: float = 1e-7,
) -> torch.Tensor:
    """Return the official GCD regression loss on paired xyxy boxes.

    This is a regression-only port of the ``gcd_loss(..., mode='exp')``
    formula from MArKkwanGuan/mmdet-GCD at 59f0843 (Apache-2.0).  Candidate
    assignment deliberately remains the native IoU TaskAlignedAssigner.
    """
    pred_xywh = xyxy2xywh(pred_bboxes)
    target_xywh = xyxy2xywh(target_bboxes)
    cx_p, cy_p, w_p, h_p = pred_xywh.unbind(-1)
    ...
    center_distance_pred = ((cx_p - cx_t) / (w_p + eps)).pow(2) + ((cy_p - cy_t) / (h_p + eps)).pow(2)
    shape_distance_pred = ((w_p - w_t) / (w_t + eps)).pow(2) + ((h_p - h_t) / (h_t + eps)).pow(2)
    center_distance_target = ((cx_p - cx_t) / (w_t + eps)).pow(2) + ((cy_p - cy_t) / (h_t + eps)).pow(2)
    shape_distance_target = ((w_p - w_t) / (w_p + eps)).pow(2) + ((h_p - h_t) / (h_p + eps)).pow(2)

    gcd_squared = (center_distance_pred + shape_distance_pred / 4 + center_distance_target + shape_distance_target / 4) / 2
    return 1.0 - torch.exp(-torch.sqrt(gcd_squared.clamp_min(0)))
...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max: int = 16, use_swd: bool = False, use_gcd: bool = False, swd_gamma: float = 0.5,
                 swd_c: float | str = "auto", swd_alpha: float = 1.0):
        """Initialize the BboxLoss module with regularization maximum and DFL settings.

        swd_alpha: 融合权重。1.0=纯 SWD（默认，向后兼容）；0.0=纯 CIoU；
                   0<alpha<1 = Hybrid: (1-alpha)*CIoU + alpha*SWD。
        """
        super().__init__()
        self.dfl_loss = DFLoss(reg_max) if reg_max > 1 else None
        self.use_swd = use_swd
        self.use_gcd = use_gcd
        self.swd_gamma = swd_gamma
        self.swd_c = swd_c
        self.swd_alpha = swd_alpha
    ...
    def forward(
        ...
        # ===== phased alpha 退火 / 两阶段硬切换（隔离设计，绝不污染其他训练）=====
        # 仅当 YOLO_SWD_ALPHA_SCHEDULE 设置时生效，支持两种格式：
        #   1) 线性退火 "start,end"        例 "0.5,0.0" —— alpha 随 epoch 线性从 start 退到 end
        #   2) 两阶段硬切换 "step,a,switch" 例 "step,0.5,100" —— 前 switch 轮 alpha=a，之后纯 CIoU(0)
        # 不设时 _alpha = self.swd_alpha，与改造前行为【一字节不变】。
        # epoch 由 train 脚本注册的 callback 写入 YOLO_CURRENT_EPOCH（仅 schedule 训练注册）。
        _alpha = self.swd_alpha
        _sched = os.getenv("YOLO_SWD_ALPHA_SCHEDULE")
        if _sched:
            try:
                _parts = [x.strip() for x in _sched.split(",")]
                _e = float(os.getenv("YOLO_CURRENT_EPOCH", "0"))
                if _parts[0].lower() == "step":
                    # 两阶段硬切换：前 switch 轮 alpha=a，第 switch 轮起纯 CIoU
                    _alpha_a, _switch = float(_parts[1]), float(_parts[2])
                    _alpha = _alpha_a if _e < _switch else 0.0
                else:
                    # 线性退火 "start,end"
                    _start, _end = float(_parts[0]), float(_parts[1])
                    _total = max(float(os.getenv("YOLO_TOTAL_EPOCHS", "300")), 1.0)
                    _frac = min(max(_e / _total, 0.0), 1.0)
                    _alpha = _start + (_end - _start) * _frac  # 线性退火
            except Exception:
                _alpha = self.swd_alpha
        if self.use_gcd:
            gcd_loss = compute_gcd_loss(pred_bboxes[fg_mask], target_bboxes[fg_mask]).unsqueeze(-1)
            loss_iou = (gcd_loss * weight).sum() / target_scores_sum
        elif self.use_swd:
            # SWD 损失项（各向异性高斯 2-Wasserstein 相似度）
            swd = compute_swd(pred_bboxes[fg_mask], target_bboxes[fg_mask], gamma=self.swd_gamma)
            if self.swd_c == "auto":
                target_xywh = xyxy2xywh(target_bboxes[fg_mask])
                norm = torch.sqrt(target_xywh[:, 2].pow(2) + target_xywh[:, 3].pow(2) + 1e-9).mean().detach()
                norm = torch.clamp(norm, min=1.0)
            else:
                norm = torch.tensor(float(self.swd_c), device=pred_bboxes.device, dtype=pred_bboxes.dtype)
            swd_sim = torch.exp(-swd / norm)
            loss_swd = ((1.0 - swd_sim).unsqueeze(-1) * weight).sum() / target_scores_sum

            if _alpha >= 1.0:
                # 纯 SWD（向后兼容：与改造前行为完全一致，alpha 默认 1.0）
                loss_iou = loss_swd
            elif _alpha <= 0.0:
                # 纯 CIoU（退化，相当于 baseline）
                iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True)
                loss_iou = ((1.0 - iou) * weight).sum() / target_scores_sum
            else:
                # Hybrid: loss_box = (1 - alpha) * CIoU + alpha * SWD
                # CIoU 主导定位精度，SWD 增强极小目标召回（HVE-7 证据支撑）
                iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True)
                loss_ciou = ((1.0 - iou) * weight).sum() / target_scores_sum
                loss_iou = (1.0 - _alpha) * loss_ciou + _alpha * loss_swd
        else:
            iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True)
            loss_iou = ((1.0 - iou) * weight).sum() / target_scores_sum
...
class v8DetectionLoss:
    ...
    def __init__(
        self, model, tal_topk: int = 10, tal_topk2: int | None = None, assignment_trace_name: str | None = None
        ...
        self.use_swd_loss = os.getenv("YOLO_USE_SWD_LOSS", "0") == "1"
        self.use_gcd_loss = os.getenv("YOLO_USE_GCD_LOSS", "0") == "1"
        if self.use_swd_loss and self.use_gcd_loss:
            raise ValueError("YOLO_USE_SWD_LOSS and YOLO_USE_GCD_LOSS cannot both be enabled.")
        self.swd_gamma = float(os.getenv("YOLO_SWD_GAMMA", "0.5"))
        self.swd_c = os.getenv("YOLO_SWD_C", "auto")
        # 融合权重：默认 1.0 = 纯 SWD（向后兼容，旧 run 续训行为不变）
        self.swd_alpha = float(os.getenv("YOLO_SWD_ALPHA", "1.0"))
        ...
        self.assigner = TaskAlignedAssigner(
            ...
            topk2=tal_topk2,
            trace_name=assignment_trace_name,
        )
        self.bbox_loss = BboxLoss(
            m.reg_max,
            use_swd=self.use_swd_loss,
            use_gcd=self.use_gcd_loss,
            swd_gamma=self.swd_gamma,
            swd_c=self.swd_c,
            swd_alpha=self.swd_alpha,
        ).to(device)
...
class E2ELoss:
    ...
    def __init__(self, model, loss_fn=v8DetectionLoss):
        ...
        self.one2many = loss_fn(model, tal_topk=10, assignment_trace_name="o2m")
        self.one2one = loss_fn(model, tal_topk=7, tal_topk2=1, assignment_trace_name="o2o")
        self.updates = 0
        self.total = 1.0
        # init gain
        self.o2m = 0.8
